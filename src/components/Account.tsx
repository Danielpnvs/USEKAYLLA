...
export default function Account({ onLogin, onLogout, isLoggedIn: propIsLoggedIn, currentUser: propCurrentUser }: AccountProps) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('🔍 Tentando login:', { rawLogin, loginEmail });
      
      // NOVA LÓGICA: LOGIN POR NOME APENAS (SEM EMAILS)
      console.log('🔍 INICIANDO LOGIN POR NOME');
      
      let baseUser = null;
      let effectivePassword = '';
      
      // 1. PRIMEIRO: Buscar no Firebase por nome (sem email)
      let firebaseUser = null;
      
      // NOVA LÓGICA: Buscar por role baseado no nome de login
      let userRole = null;
      ...
      if (knownNames[rawLogin as keyof typeof knownNames]) {
        ...
        // Tentar encontrar usuário com esse nome em qualquer role
        const allUsers = await Promise.all([
          getUserByEmail('admin@usekaylla.com'),
          getUserByEmail('user@usekaylla.com'),
          getUserByEmail('test@usekaylla.com')
        ]);
        
        const foundUser = allUsers.find(user => 
          user && user.name.toLowerCase() === rawLogin.toLowerCase()
        );
        
        if (foundUser) {
          userRole = foundUser.role;
          mappedEmail = foundUser.email;
          console.log('🔍 Nome encontrado no Firebase:', rawLogin, '→', userRole, 'Email:', mappedEmail);
        } else {
          console.log('❌ Nome não encontrado no Firebase:', rawLogin);
        }
      }
      
      if (userRole && mappedEmail) {
        firebaseUser = await getUserByEmail(mappedEmail);
        console.log('🔥 Firebase user encontrado:', firebaseUser ? 'Sim' : 'Não');
        if (firebaseUser) {
      ...
      // 2. FIREBASE SEMPRE PRIMEIRO (admin e usuário salvam aqui)
      if (firebaseUser) {
        // Usar dados do Firebase (onde tanto admin quanto usuário salvam)
        baseUser = {
        ...
        baseUser = USERS.find(u => u.role === userRole);
        console.log('📋 Fallback - Array USERS');
        
        if (baseUser) {
          effectivePassword = baseUser.password;
          console.log('🔐 Usando dados do array USERS');
        }
      }
  ...
  const verifyCurrentPassword = async (password: string): Promise<boolean> => {
    try {
      ...
      if (firebaseUser) {
        ...
        // Verificar senha do array local
        const localUser = USERS.find(u => u.email === currentUser?.email);
      ...
      try {
        ...
        if (userIndex !== -1) {
          USERS[userIndex] = {
            ...
            name: selfEditForm.name,
            email: newEmail,
            password: selfEditForm.newPassword || USERS[userIndex].password
          };
        }
        ...
        // Se for um usuário normal, atualizar dados visíveis ao admin
        if (currentUser?.role === 'user') {
          localStorage.setItem('usekaylla_user_data', JSON.stringify({
            name: selfEditForm.name,
            email: newEmail,
            password: selfEditForm.newPassword || USERS.find(u => u.role === 'user')?.password || '',
            role: currentUser?.role
          }));
        }
        ...
        setSelfEditForm({ name: '', email: '', currentPassword: '', newPassword: '', confirmPassword: '' });
      } catch (error) {
  ...
  const handleEditUser = async () => {
    try {
      ...
      const userEmail = 'user@usekaylla.com';
      ...
      if (firebaseUser) {
        ...
        if (userUser) {
          ...
          console.log('⚠️ Fallback para dados originais do user');
        }
      }
    } catch (error) {
  ...
  // Função para editar usuário test/viewer - ADMIN TEM PRIORIDADE ABSOLUTA
  const handleEditTest = async () => {
    try {
      // ADMIN SEMPRE BUSCA DADOS ATUALIZADOS DO FIREBASE
      const testEmail = 'test@usekaylla.com';
      const firebaseUser = await getUserByEmail(testEmail);
      
      console.log('🔍 Admin buscando dados do test no Firebase...');
      ...
      if (firebaseUser) {
        ...
        if (testUser) {
          ...
          console.log('⚠️ Fallback para dados originais do test');
        }
      }
    } catch (error) {
      console.error('Erro ao buscar dados do usuário test:', error);
      // Fallback para dados originais
      const testUser = USERS.find(u => u.role === 'viewer');
      if (testUser) {
        setUserEditForm({
  ...
  const handleSaveUserEdit = async () => {
    if (userEditForm.name && userEditForm.password) {
      try {
        ...
        console.log('💾 ADMIN SALVANDO E SINCRONIZANDO:', { name: userEditForm.name });
        ...
        // 2. ATUALIZAR ARRAY USERS (compatibilidade)
        const userIndex = USERS.findIndex(u => u.role === userRole);
        if (userIndex !== -1) {
          USERS[userIndex] = {
            ...USERS[userIndex],
            name: userEditForm.name,
        ...
        // 3. ATUALIZAR LOCALSTORAGE (sincronização)
        if (isTest) {
        ...
        // 4. LIMPAR CREDENCIAIS PARA FORÇAR USO DO FIREBASE
        const creds = readCredentials();
        delete creds[userEmail];
        writeCredentials(creds);
        
        console.log('✅ SINCRONIZAÇÃO COMPLETA - Nova senha já funciona para login');
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      // NOVA HIERARQUIA: Usuário pode mudar senha e salva no Firebase
      // Admin pode sobrescrever a qualquer momento
      console.log('👤 USUÁRIO MUDANDO SENHA - Salvando no Firebase');
      
      // Atualizar no Firebase (usuário tem autonomia para senha)
      if (firebaseUser) {
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full">
              <div className="p-6 border-b border-gray-200">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-bold text-gray-900">
                    {userEditForm.email === 'test@usekaylla.com' ? 'Editar Test' : 'Editar Usuário (User)'}
                  </h3>
                  <button
            ...
            <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 p-6 text-center">
              ...
              <h3 className="text-2xl font-bold text-green-600 mb-2">
                {successType === 'name' ? 'Nome Atualizado!' : 
                 ...
                 successType === 'user_updated' ? 'Usuário Atualizado!' : 'Sucesso!'}
              </h3>
            ...
            <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 p-6 text-center">
              <div className="text-6xl mb-4">
                {errorType === 'password_incorrect' ? '🔒' : 
                 ...
                 errorType === 'user_fields_required' ? '📝' : '⚠️'}
              </div>
              <h3 className="text-2xl font-bold text-red-600 mb-2">
                {errorType === 'password_incorrect' ? 'Senha Incorreta!' : 
                 ...
                 errorType === 'user_fields_required' ? 'Campos Obrigatórios!' : 'Erro!'}
              </h3>
              <p className="text-gray-600 mb-6">
                {errorType === 'password_incorrect' 
